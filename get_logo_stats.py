...
import os
import json
from src.utils.generators.logos_gen import LogosGen
...
def infer_programs(imitate_net, self_training=False, ab=None):
    ...
    generator = LogosGen("data/noun-images.npy")
    test_gen = generator.get_train_data()
    ...
    for batch_idx in range(config.test_size // config.batch_size):
        with torch.no_grad():
            ...
            target_images = data_.astype(dtype=bool)
