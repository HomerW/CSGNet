...
from src.utils.train_utils import prepare_input_op, cosine_similarity, chamfer, beams_parser, image_from_expressions
import time
...
def get_cd(imitate_net, data, one_hot_labels, program_len):
    ...
    beam_labels = beams_parser(
        all_beams, batch_size, beam_width=beam_width)

    beam_labels_numpy = np.zeros(
        (batch_size * beam_width, program_len), dtype=np.int32)
...
for epoch in range(config.epochs):
    ...
    #print("Epoch {}/{}=>  train_loss: {}, iou: {}, cd: {}, test_mse: {}, test_acc: {}, beam_cd: {}".format(epoch, config.epochs,
    #                                  mean_train_loss.cpu().numpy(),
    #                                  metrics["iou"], metrics["cd"], test_loss, acc, beam_CD))
    print("Epoch {}/{}=>  train_loss: {}, cd: {}".format(epoch, config.epochs,
                                      mean_train_loss.cpu().numpy(), metrics["cd"]))
    ...
    if ((epoch + 1) % 10) == 0:
        torch.save(imitate_net.state_dict(), f"trained_models/synthetic_benchmark_epoch{epoch}.pth")
