...
class MixedGenerateData:
    def __init__(self,
        ...
        with open("perturbations", "rb") as f:
            self.perturbations = pickle.loads(f.read())
        ...
        while True:
            ...
            for rand_id in range(0, num_train_images - batch_size, batch_size):
                ...
                for index, value in enumerate(image_ids):
                    ...
                    perturb = self.perturbations[program_len][value]
                    new_program = []
                    for token, p in zip(program, perturb):
                        new_token = {}
                        if token['type'] == 'draw':
                            token['param'] = [int(x) + delta for x, delta in zip(token['param'], p)]
                        new_program.append(token)
                    sim.generate_stack(new_program)
                    stack = sim.stack_t
